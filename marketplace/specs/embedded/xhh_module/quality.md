...
需要用户后续补充或修改的地方，用 `TODO` 标注。`TODO` 必须是单行注释中的第一个词；`//` 后允许格式化工具插入空格，便于保持代码格式一致并能全局定位。
...
约定：
- `//TODO`、`// TODO` 或带冒号的等价形式开头；`//` 与 `TODO` 之间允许空格
- 推荐用 `rg -n '^\s*//\s*TODO(?::)?\b'` 搜索，兼容格式化后的空格
- 一行只标注一个 TODO 事项
...
README 记录已确认的硬件和产品事实；源码注释只说明代码行为、非直观原因和修改约束，不能成为第二份硬件或产品说明书。

- 禁止在注释中重复记录 MCU 型号、板卡版本、端口/引脚、有效电平、ADC 通道、外设实例、产品功能映射或产品标定事实；这些信息以 README、原理图和实际代码为准。
- `xhh_BSP/*.h` 的 Doxygen 只记录稳定接口契约、参数范围、返回值和前置条件，不写当前平台或板卡事实。
- `xhh_BSP/*.c` 可说明不可从代码直接推导的硬件限制、芯片勘误、强制时序和异常处理原因；不得用注释重复罗列资源映射。
- `xhh_Module` 注释保留业务状态、事件、周期、算法和数值语义；不得重复 MCU、端口、引脚、极性或厂家资源。
- `Components` 注释保留器件协议、命令时序和数据处理；不得描述产品状态、Task/Event 语义或板卡连接事实。
- 占位函数仍必须用 `AI:` 注释说明当前行为和启用条件，但使用“当前实现”描述，不写具体板卡或产品名称。
- 单函数内的裸数值注释可说明单位、计算含义和修改条件；不重复列出已由 README/原理图维护的资源映射。

---

## 数值配置位置（强制）

- 只在单个函数使用、且修改时需要同时理解业务语义的少量数值，不定义为文件级 `#define`；直接写在使用分支或计算式中，并用中文 `AI:` 注释说明数值的单位、物理/业务含义、适用区间和修改条件。
- 仅服务一个函数的分格阈值、标定曲线等多项数据，使用该函数内的 `static const` 表；表前注释必须说明排序、每个元素对应的边界，以及修改表时需要同步核对的等级数或资源数。
- 跨函数共享的协议字段、公开接口常量、编译开关、数组容量和硬件寄存器位等，才使用命名宏、枚举或文件级 `static const`。
- 禁止无注释的裸数值；需要调整的标定值不得隐藏在与业务位置无关的宏定义区。

```c
static uint32_t xhh_Task_BAT_Charge_MARK(uint32_t voltage_mv)
{
	/* AI:充电抬升回扣量，单位 mV；91%~100% 的实测值为 55mV。 */
	if (val > 90U)
	{
		voltage_mv -= 55UL;
	}
