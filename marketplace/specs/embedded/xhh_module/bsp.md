...
- 新增、删除或调整硬件资源时，必须保持资源归属、对应 BSP Init 与 `xhh_BSP_SYS_Init` 时钟清单一致。
...
- 仅当 MCU 没有可用 SysTick，或 SysTick 已被其他时基独占且无法安全复用时，才允许使用硬件 Timer 实现 `1ms` 节拍；实现注释只说明无法复用 SysTick 的原因。
...
- 回调默认只执行置位、计数等短小且确定时间的操作；业务逻辑由主循环消费状态后执行。
- 已确认必须在中断周期内更新的硬件输出，可由回调调用经过评审的 `xhh_Task_<X>_Out` 接口。该接口必须在 Doxygen 标注 ISR 上下文，只执行确定时间的硬件输出；不得调用 `_Loop`、Event Handle、状态机、Flash、日志或阻塞接口。
...
- 采样对象到逻辑槽位的映射属于项目硬件事实；业务代码只使用已确认的槽位。
...
- `IT_Disable/IT_Enable` 使用嵌套深度，并保存最外层进入前的中断状态，防止内层临界区退出时提前打开中断；必须可从主循环或 ISR 调用，并在 ISR 内恢复进入临界区前的中断屏蔽状态。
...
新增硬件能力时，按以下优先级选择源码归属：

1. 归入已有基础接口，例如 GPIO、PWM、ADC。
2. 扩展既有 BSP 类别的最小公开接口。
3. 确认是新的基础外设能力，例如 I2C、SPI，才新增 `xhh_BSP_<能力>.h/.c`。
