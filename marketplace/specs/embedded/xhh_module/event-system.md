...
| 组成 | 形式 |
|------|------|
| 事件值 | Event 文件私有 `static volatile xhh_event_slot` |
| 事件参数 | Event 文件私有 `static volatile xhh_event_parameter_slot` |
| 触发 | `xhh_Event_Trigger(event, parameter)` 在临界区写入单槽 |
| 分发 | `xhh_Event_Handle()` 在临界区复制事件和参数快照、清空事件槽后进入 `switch` |

单槽设计:同一时刻只持有一个事件。新事件会覆盖未处理的旧事件。这是有意的(嵌入式场景事件密度低),不要改成队列。`xhh_Event_Trigger` 允许从主循环或 ISR 调用；单槽读写必须由 `xhh_BSP_SYS_IT_Disable/IT_Enable` 成对保护，保证事件和参数作为同一份快照发布与取出。
...
`xhh_Event_Handle()` 取出事件后,**必须先把来源和数据字节都提取好**,即使这个事件可能用不到数据。来源判断只能比较 `source_id`，不得将携带低 16bit 数据的完整 `param` 与来源 ID 全等比较:
...
void xhh_Event_Handle(void)
{
    ...
    xhh_event_slot = xhh_Event_Null;             // AI:取出即清空，后续新事件进入下一轮处理。
...
{
    /* AI:低 16bit 可携带触摸业务数据，来源仅比较高 16bit。 */
}
```
...
关键点:
- `Trigger` 只做临界区内单槽写入，不打印日志、不调用 Task/Mode/Flash，因此可以从 ISR 调用。
- `Handle` 在临界区内**取出并清空** `xhh_event_slot`，避免事件和参数错配。
...
| ISR 安全事件 | 中断内完成事件与参数计算后直接 Trigger；不可在 ISR 内 Handle |
...
- `Handle` 里不提取参数直接 switch(必须先提取 `source_id/data_16/data_h/data_l`)
- 使用完整 `param` 与来源 ID 全等比较；来源带低 16bit 数据时必须比较 `source_id`
- 在 `xhh_Event_Trigger` 内打印日志、调用业务接口或执行其他可能阻塞的操作
