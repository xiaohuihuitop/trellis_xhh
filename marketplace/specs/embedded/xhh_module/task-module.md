...
Task 可以直接调用其他 Task 的公开接口，不要求完全隔离；例如 `xhh_Task_BAT` 在电量变化后调用 `xhh_Task_UI` 刷新显示。直接调用必须同时满足以下条件：

- 调用方向单向明确，目标 Task 不得反向调用调用方，禁止递归和回调环。
- 调用只完成当前业务链的局部更新，不直接调用 `xhh_SYS_Change`、不改变全局系统状态。
- 协议入口、按键入口、BLE 入口，以及需要集中编排多个 Task 的业务转换，仍必须通过 `xhh_Event`。
- 发生状态迁移、持久化、超时复位等跨域副作用时，由 Event case 集中协调，避免同一业务流程分散在多个 Task 中。
