...
业务逻辑（Event Handle、状态机推进、Flash 操作）必须在主循环运行，不进中断。`xhh_Event_Trigger` 是受限例外：只写入预先计算好的事件与参数，可在 ISR 内调用。
...
us~ms 级必须立即响应的输出，可以在中断里直接调用已明确标注 ISR 安全的 `xhh_Task_<X>_Out()`：

- Motor PWM 输出（`xhh_Task_Motor_Out()`）
- 已确认的 LED 单次输出（不得调用 `xhh_Task_LED_Loop()`）
- LCD 刷新
...
- ✅ 中断里**可以 `xhh_Event_Trigger(xhh_Event_ERR, ...)` 产生事件**；参数必须已在 ISR 内计算完成，Trigger 本身不得打印日志或调用业务接口
...
- ❌ 大块数据处理 / 长循环
- ❌ 调用未标注 ISR 安全的 `xhh_Task_*` 接口，尤其是任何 `_Loop`
