...
- Git 提交完成后，单独询问是否进行 Trellis 收尾。收尾授权只包含任务归档和会话记录；归档或日志提交仍需单独展示计划并取得确认，不包含推送。
- 推送必须由用户单独明确要求。
- 外部、实物或用户验收尚未完成时默认不收尾；只有用户明确允许带未验证项收尾时才可继续，并在 `result.md` 保留未验证内容。
- 修改范围需要扩大时，先更新范围和验证计划并取得用户确认。
- 任务完成后必须先向用户报告实际修改、原有行为回归、范围外差异和未验证项，并取得“任务完成且结果通过”的明确确认。该确认只允许进入提交计划，不包含 Git 提交、归档或推送授权。
- 进入任何会写入任务归档、会话日志或索引的命令前，先读取 `.trellis/config.yaml` 并确认 `session_auto_commit: false`。配置缺失、为 `true` 或无法解析时，停止该命令并报告；不得依赖 Trellis 脚本的默认值，也不得用 `--no-commit` 之外的方式绕过检查。
- 任何 `git commit`（包括工作代码提交、归档提交和日志提交）都必须在同一任务内先展示具体文件清单和提交信息，并取得用户明确确认。实施代理、检查代理和收尾流程均不得自行提交；发现已有暂存文件不属于本次确认清单时，先停止并报告。
- 代理状态门禁：代理尚未回传最终结果、只输出请求/启动文字、上下文清单仍只有 `_example`/seed，或没有绑定实际源码版本和检查输出时，不得报告质量门通过，也不得进入“任务完成且结果通过”确认。

授权含义示例：

| 用户表达 | 允许动作 | 不允许动作 |
|---|---|---|
| “开始做” | 在已确认范围内实施和验证 | 提交 Git、归档、推送 |
| “功能做完了，结果通过” | 展示精确提交计划 | 直接提交或归档 |
| “按这个提交计划提交” | 仅提交展示的文件和信息 | 修改文件清单、顺带提交其他暂存内容 |
| “可以归档了” | 归档任务和记录会话 | 推送远端、改变未确认内容 |
| “继续”或“好” | 若上下文明确，继续当前已授权阶段 | 把模糊回应解释成提交或归档授权 |
...
- 3.4 提交修改 `[required · once, result-confirmed-and-user-confirmed]`
- 3.5 收尾提醒
...
有实际修改时，完成最终全范围检查和 `result.md` 后，先向用户报告任务结果并等待“任务完成且结果通过”的明确确认；未取得该确认不得准备或执行提交：
...
4. 检查暂存区：若存在未由本任务确认的暂存内容，先停止，不得通过无路径限定的 `git commit` 继续。
5. 按一个提交只解决一个问题的原则拟定提交计划，列出提交信息和文件。
6. 输出实际结果摘要，明确请求用户确认“任务完成且结果通过”。用户未确认、提出疑问或要求继续检查时，停留在当前任务，不得提交或归档。
7. 用户确认结果通过后，才输出 `[trellis] 阶段=提交 动作=等待用户确认`，展示精确提交计划并等待提交授权。
8. 获得提交授权后只执行已展示的提交；使用带明确路径的 `git add` 和 `git commit`，禁止 amend 和推送。用户拒绝时停止。
9. 提交完成后输出 `[trellis] 阶段=收尾 动作=等待用户确认` 并停止，不在同一轮执行归档或日志提交。

#### 3.5 收尾提醒

用户明确同意收尾后，输出 `[trellis] 阶段=收尾 动作=执行`，再执行 `/finish-work` 或等价的官方收尾流程。该授权只包含任务归档和会话记录；归档/日志产生的 Git 提交仍必须遵守 `session_auto_commit: false`，并在执行前展示文件清单和提交计划、取得用户确认，不包含推送。
